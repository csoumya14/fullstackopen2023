...
sequenceDiagram
    ...
    Note right of server: The browser do a HTTP POST request to the server address new_note

    server-->>browser: HTTP status code 302
    deactivate server

    Note right of browser: The server asks the browser to do a HTTP GET request to the address notes
